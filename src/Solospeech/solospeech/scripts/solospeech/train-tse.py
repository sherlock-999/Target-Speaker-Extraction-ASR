...
# model configs
parser.add_argument('--diffusion-config', type=str, default='./config/SoloSpeech-tse-base.yaml')
...
if __name__ == '__main__':
    ...
    if accelerator.is_main_process:
        ...
        eval_ddim(model_module, autoencoder, noise_scheduler, eval_loader, args, accelerator.device, epoch='test', ddim_steps=args.num_infer_steps, eta=0)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_loader)):
            ...
            reference_lengths = reference_lengths.to(accelerator.device)
            ...
            # inference
            pred, pred_mask = model(x=noisy_target, timesteps=timesteps, mixture=mixture, reference=reference, x_len=lengths, ref_len=reference_lengths)
            # backward
            if args.v_prediction:
        ...
        if accelerator.is_main_process:
            model_module = model.module if hasattr(model, 'module') else model
            eval_ddim(model_module, autoencoder, noise_scheduler, eval_loader, args, accelerator.device, epoch=epoch+1, ddim_steps=args.num_infer_steps, eta=0)
